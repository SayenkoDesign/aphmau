 ...
 *
 * Copyright 2016 The WooTax Corporation (email: support@wootax.com)
 *
...
final class WC_WooTax {
	...
	private function frontend_includes() {
		if ( $this->get_option( 'show_exempt' ) == 'true' ) {
			require_once 'includes/frontend/wc-wootax-exemptions-frontend.php';
		}
	...
	private function has_dependencies() {
		if ( ! class_exists( 'SoapClient' ) ) {
			return false;
		} else if ( ! class_exists( 'WooCommerce' ) || version_compare( WOOCOMMERCE_VERSION, '2.2', '<' ) ) {
			return false;
		}
	...
	public function activate() {
		...
		if ( $this->has_dependencies() ) {
			...
			wootax_add_message( '<strong>Simple Sales Tax could not be activated.</strong> Please ensure that PHP SOAP is supported by your server, and check that WooCommerce 2.2 or greater is installed.', 'error', 'activate-error', true, true );
