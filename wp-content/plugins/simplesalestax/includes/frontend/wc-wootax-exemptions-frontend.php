...
require_once trailingslashit( dirname( __FILE__ ) ) .'class-wt-exemption-certificate.php';
...
function maybe_display_exemption_link() {
	...
	if ( SST()->get_option( 'show_exempt' ) == 'true' ) {
		...
		// Get URL of manage certificates template (this is displayed by default)
		$manage_certs_url = admin_url( 'admin-ajax.php?action=wootax-load-template&template=manage-certificates' );
		$notice = "Are you a tax exempt customer? <a class='popup-link mfp-iframe' id='wootax_exemption_link' href='$manage_certs_url'>$link_text</a>";
		...
		$message_class = version_compare( WOOCOMMERCE_VERSION, '2.3', '<' ) ? 'woocommerce-info' : 'woocommerce-message';
...
function ajax_load_exemption_template() {
	// Get template name and query vars
	...
	// Load template using wc_get_template
	die( wc_get_template( $template_name, array_merge( array(
		'plugin_url'   => SST()->plugin_url(),
		'company_name' => SST()->get_option( 'company_name' ),
	), $query_vars ), 'sst/lightbox/', SST()->templates_path() .'/lightbox/' ) );
}
...
function enqueue_checkout_scripts() {
	if ( !is_admin() && is_checkout() ) {
		...
		// Enqueue checkout JS
		wp_enqueue_script( 'sst-checkout', SST()->plugin_url() .'/js/sst-checkout.js', array( 'jquery', 'mpop-js' ), '1.0', true );
		wp_enqueue_script( 'cert-manager', SST()->plugin_url() .'/js/certificate-manager.js', array( 'jquery' ), '1.0', true );
	}
}
